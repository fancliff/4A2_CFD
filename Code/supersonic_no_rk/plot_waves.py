...
def main():
    ...
    (cp_error, cpstag_error, mach_error) = calc_waves_error(av,g,p_ref,pstag_ref)

    print(f'Cp error: {cp_error:.3e}')
    print(f'Cpstag error: {cpstag_error:.3e}')
    print(f'Mach error: {mach_error:.3e}')

    # Specify the parameters to plot
    fieldnames = ['cp', 'cp_exact', 'mach', 'M_exact', 'cpstag']; 
    colnames = ['Static pressure coefficient', 'Exact static pressure coefficient', 'Mach number', 'Exact Mach Number', 'Stagnation pressure coefficient']
